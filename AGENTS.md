...
- Round-trip тесты (`QuickCProgramRoundTripTests`) требуют DOSBox с QuickC; известные сбои перечислены в `QuickC/PROGRAMS/roundtrip_xfail.txt`.
...
- **При доработке кодогенерации** — проверяй и unit-тесты (`CCodeGeneratorTests`, `*DecompileTests`), и round-trip (если доступен DOSBox).
