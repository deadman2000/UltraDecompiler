...
1. Установить [DOSBox-X](https://dosbox-x.com/) и добавить `dosbox-x.exe` в `PATH`, **или** задать переменную окружения `DOSBOX_X_PATH` с полным путём к исполняемому файлу.
2. Проверка: `where dosbox-x` (Windows) или `DosBoxQuickCAssets.IsDosBoxAvailable` в тестах.
